...
검증('아주 먼 저점은 마지노선에서 제외한다', () => {
    // 월봉처럼 상장 초기까지 닿는 봉을 섞으면 현재가 -96% 저점이 후보에 들어온다.
    // 그런 값을 "최후 방어선"이라 부르면 이탈 경고가 의미를 잃는다.
    const 최근 = 캔들([...경로(95, 105, 40), ...경로(105, 98, 20)]);
    const 초기 = 캔들([...경로(4, 100, 40), ...경로(100, 98, 20)]);   // 4까지 내려가는 장기봉
    const r = LevelEngine.analyze({ '1h': 최근, '1M': 초기 }, 100);
    assert.ok(r.마지노선, '마지노선이 있어야 한다');
    assert.ok(r.마지노선.price >= 100 * 0.55,
        `마지노선 ${r.마지노선.price}는 현재가 -45% 안이어야 한다`);
    assert.notStrictEqual(r.마지노선.tf, '1M', '먼 장기봉 저점이 뽑히면 안 된다');
});

검증('전부 멀면 그중 가장 가까운 것을 쓴다', () => {
    // 급등 직후처럼 모든 저점이 멀 수도 있다. 그때 마지노선을 비우면 안 된다.
    const 얕은 = 캔들(경로(30, 50, 40));
    const 깊은 = 캔들(경로(5, 50, 40));
    const r = LevelEngine.analyze({ '1h': 얕은, '1d': 깊은 }, 100);   // 현재가가 한참 위
    assert.ok(r.마지노선, '비우지 말고 대체값을 써야 한다');
    assert.ok(r.마지노선.price >= 29, `가장 가까운 저점(30)이어야: ${r.마지노선.price}`);
});
